    ...
    <SeasonsList>
        {seasons.sort((a, b) => a.season_number - b.season_number).map(season => (
            <Season key={season.id}>
                ...
                <ImageContainer>
                    <Image bgUrl={season.poster_path ? `https://image.tmdb.org/t/p/w300${season.poster_path}` : require("../../assets/no_image.jpg").default} />
                </ImageContainer>
                <InfoConatiner>
                    <AirDate>{season.air_date ? season.air_date.substring(0,4) : "Not Air"}</AirDate>
